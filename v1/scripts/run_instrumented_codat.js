// Combines instrument + instrument2 (walker + find counters), then runs full pipeline for codatplatform.
require('./instrument.js');
require('./instrument2.js');
...
async function run(caseName, version, spec, format) {
  ...
  const stepSpec = {
    spec: { base: Path.normalize(TOP_FOLDER), buildargs: { apidef: { ctrl: { step: { parse: true, guide: true, transformers: true, builders: true, generate: true } } } } }
  };
  const t = Date.now();
