...
PROJECT = "Voxgig Apidef Validate"

# The vocabulary directory under .vale/styles/config/vocabularies/.
VOCAB = "ApidefValidate"

# The root pages, always required. The README is the only reader-facing
# page: the four kinds of documentation for apidef itself live in the
# apidef repository, and this one holds a corpus and a harness.
ROOT_PAGES = ("README.md",)

# The documentation tree. None here.
DOC_GLOBS = ()
NOT_DOCS = ()

# Pages outside the tree. None: v1/guide/README.md is a two-line note
# labelling the golden guide files beside it, not a page a reader lands on,
# and v1/ has no README of its own.
EXTRA_PAGES = ()

# Multi-port layout (one directory per language, each carrying its own
# pages). Off here: the TypeScript and Go harnesses sit under v1/.
PORT_DIRS = False
NOT_PORTS = ()
PORT_PAGES = ()

# Where "we" is allowed: the tutorial, which walks through code with the
# reader (voice rule 7). There is no tutorial here.
TUTORIAL_PAGES = ()
